...
本仓库是「小树成长」单仓：现阶段只有 H5，后期会加 Node.js 与微信小程序。改代码前先读本文件和 `docs/`。

## 现在改哪里

- **H5 UI / 路由 / Pinia / localStorage**：仓库根 `src/`，技术栈保持 Vue3 + Vite + TS + Pinia + Vue Router。
...
不要把级联删除、共享课、统计口径只写在页面组件里；Store 保持业务语义，契约保持与 `src/domain/types.ts`、`src/stores/app.ts` 一致。

## 以后改哪里（尚未落地）

- H5 工程迁入 `apps/web/`（另一次提交，不在本次）。
